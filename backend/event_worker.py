...
class EventWorker(BaseWorker):
    ...
    def _flush_batch(self, ch):
        """Flush buffered events to database, cache, and evaluate rules."""
        if not self._event_buffer:
            self._last_flush_time = time.time()
            return

        events_to_flush = self._event_buffer[:]
        acks_to_send = self._pending_acks[:]
        ...
        try:
            ...
            try:
                ...
                count = repo.create_batch(events_to_flush)
                self._logger.info("Batch persisted %d events to database", count)
            finally:
                db.close()

            affected_sources = set(e["source_name"] for e in events_to_flush)
            self._evaluate_rules_for_sources(affected_sources, ch)

            for delivery_tag in acks_to_send:
    ...
    def _evaluate_rules_for_sources(self, source_names, channel):
        """Evaluate all rules for the given sources and publish triggered alerts to queue."""
    ...
    def _run_loop(self):
        """Custom run loop with periodic flushing."""
